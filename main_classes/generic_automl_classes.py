...
while glycoletter_lib is None:
    ...
    try:
        filename = glob.glob(dir + '/**/glycoletter_lib.pkl', recursive = True)
        with open(filename[0], 'rb') as file:
            glycoletter_lib = pickle.load(file)
...
def augmentData(df_data_input, df_data_output, sequence_type, augment_data):
    ...
    def add_unique_seq(aug_seqs, y_labels, orig_seqs):
        cleaned_comp_seqs = [] # these few lines are about making sure our new sequences are unique- not in real train set
        cleaned_y_labels = []
        for x,y in zip(aug_seqs,list(y_labels)):
            if x not in orig_seqs:
                cleaned_comp_seqs.append(x)
                cleaned_y_labels.append(y)
        return(cleaned_comp_seqs, cleaned_y_labels)
    ...
    if augment_data in AUGMENTATION_TYPES:
        ...
        if augment_data == 'complement':
            ...
            more, more_output = add_unique_seq(more, list(df_data_output), df_data_input)
            ...
            more, more_output = add_unique_seq(more, more_output, df_data_input)
            print('Confirmed: Augmentation of type=' + str(augment_data) + ' concluded.')        
        else: # equals none
...
def read_in_data_file(data_path, input_col, target_col):
    """read input data in csv, .xls, or .xlxs format
    Parameters
    ----------
    data_path : str representing full path where data is stored
    ...
    try:
        df_data_input = data[input_col]
    except:
        raise ValueError('Input column ' + str(input_col) + ' does not exist in the data')
    try:
        df_data_output = data[target_col]
    except:
        raise ValueError('Target column ' + str(target_col) + ' does not exist in the data')
    return(df_data_input, df_data_output, data)

def convert_generic_input(df_data_input, df_data_output, pad_seqs, augment_data, sequence_type, model_type):
    ...
    oh_data_input, numerical_data_input = AutoMLBackend.onehot_seqlist(df_data_input, sequence_type, alph, model_type = model_type)
    scrambled_oh_data_input, scrambled_numerical_data_input = AutoMLBackend.onehot_seqlist(scrambled_df_data_input, sequence_type, alph, model_type = model_type)
...
class AutoMLBackend(object): 
    ...
    def read_in_data(self):
        ...
        data : pandas DataFrame of original file
        """
        df_data_input, df_data_output, data = read_in_data_file(self.data_path, self.input_col, self.target_col)
        return df_data_input, df_data_output, data
...
class AutoMLClassifier(AutoMLBackend): 
    ...
    def bin_train_architecture_helper(self, X, y, transform_obj,seed,alph,output_folder_mod,scrambled=False, subset = None):
        """run kfold cross-validation over the best classification models
        ...
        alph : list representation of alphabet
        output_folder_mod : string modifying the path
        scrambled : boolean indicating if data is scrambled or not
        subset : str indicating dataset size if performing a data ablation test

        Returns
        -------
        compiled_seqs : list of sequences in order of targets
        compiled_true : list of ground-truth targets
        compiled_preds : list of predicted targets
        """ 
        ...
        # now get the compiled metrics and generate an overall plot 
        compiled_metrics, metric_names = self.classification_performance_eval(self.output_folder+output_folder_mod, np.array(compiled_true), np.array(compiled_preds), file_tag='compiled', display = self.verbosity)
        ...
        if subset is None:
            results_file_path = self.write_results(metric_names, avg_metric_folds, std_metric_folds, compiled_metrics, cv_scores, scrambled=scrambled)
        else:
            results_file_path = self.write_results(metric_names, avg_metric_folds, std_metric_folds, compiled_metrics, cv_scores, scrambled=scrambled, subset=subset)
        return(compiled_seqs, compiled_true, compiled_preds)  
...
class AutoMLRegressor(AutoMLBackend): 
    ...
    def reg_train_architecture_helper(self, X, y, transform_obj,seed,alph,output_folder_mod,scrambled=False, subset = None):
        """run kfold cross-validation over the best regression models
        ...
        _, _, compiled_r2, _, _ = self.regression_performance_eval(self.output_folder+output_folder_mod, np.array(compiled_true), np.array(compiled_preds), file_tag='compiled', display = self.verbosity)
        ...
        # write results to a text file for the user to read
        if subset is None:
            results_file_path = self.write_results(avg_r2_folds, std_r2_folds, compiled_r2, cv_scores, scrambled=scrambled)
        else:
            results_file_path = self.write_results(avg_r2_folds, std_r2_folds, compiled_r2, cv_scores, scrambled=scrambled, subset=subset)
        return(compiled_seqs, compiled_true, compiled_preds) 
