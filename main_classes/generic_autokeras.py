...
from interpret_helpers import plot_mutagenesis, plot_rawseqlogos
...
class AutoKerasClassification(AutoMLClassifier): 
    ...
    def convert_input(self): 
        ...
        return convert_generic_input(self.df_data_input, self.df_data_output, self.pad_seqs, self.augment_data, self.sequence_type, model_type = 'autokeras')
    ...
    def run_system(self): 
        ...
        with suppress_stdout(self.verbosity):
            ...
            compiled_seqs, compiled_true, compiled_preds = self.bin_train_architecture_helper(X, y, transform_obj,seed,alph, output_folder_mod='autokeras_')
...
class AutoKerasRegression(AutoMLRegressor): 
    ...
    def run_system(self): 
        ...
        with suppress_stdout(self.verbosity):
            ...
            # run kfold cv over the resulting pipeline
            compiled_seqs, compiled_true, compiled_preds = self.reg_train_architecture_helper(X, y, transform_obj,seed,alph, output_folder_mod='autokeras_')
        ...
        if self.dataset_robustness:
            ...
            while dataset_size > 1000:
                ...
                with suppress_stdout(self.verbosity):
                    # run kfold cv over the resulting pipeline
                    _, _, _ = self.reg_train_architecture_helper(X[0:dataset_size], y[0:dataset_size], transform_obj,seed,alph, output_folder_mod='robustness/' + str(dataset_size) + '_', scrambled = False, subset = str(dataset_size))

                    # test scrambled control on best architecture
                    _, _, _ = self.reg_train_architecture_helper(scr_X[0:dataset_size], y[0:dataset_size], transform_obj,seed,alph, output_folder_mod='robustness/scrambled_' + str(dataset_size) + '_', scrambled = True, subset = str(dataset_size))
